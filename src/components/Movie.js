...
const Movie = (props) => {
  ...
  // bu son üçünün yaptığı iş aynı, çözdükleri işlerin complexitesi farklı. ilki datayı tuttuğum fınks, ikincisi datayı değiştirdiğim fonks.

  const movies = useSelector((state) => state.movies);
  const movie = movies.find((movie) => movie.id === Number(id));

  const handleDelete = (id) => {
    dispatch(deleteMovie(id)); // sildikten sonra beni movies e geri push layacak.
    push("/movies");
  };
